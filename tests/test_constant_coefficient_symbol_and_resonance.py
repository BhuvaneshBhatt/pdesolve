...
def test_symbol_layer_reports_shift_and_lowest_term():
    ...
    u = sp.Function("u")
    eq = sp.Eq(
        sp.diff(u(x, t), t, 2) - 2 * sp.diff(u(x, t), t, x) + sp.diff(u(x, t), x, 2), 0
    )
...
def test_resonance_multiplicity_lifts_exponential_ansatz():
    ...
    u = sp.Function("u")
    eq = sp.Eq(
        sp.diff(u(x, t), t, 2) - 2 * sp.diff(u(x, t), t, x) + sp.diff(u(x, t), x, 2),
        sp.exp(x + t),
    )
...
def test_factor_pipeline_metadata_is_attached():
    ...
    u = sp.Function("u")
    eq = sp.Eq(
        (sp.diff(u(x, t), t) + 2 * u(x, t)) - (sp.diff(u(x, t), x) - u(x, t)), sp.exp(t)
    )
    result = invert_factored_constant_coefficient_operator_on_forcing(
        eq, u(x, t), (x, t)
    )
...
def test_multivariate_polynomial_trig_forcing_is_supported():
    ...
    assert (
        sp.simplify(
            _residual(
                eq, sp.expand(result.solution.rewrite(sp.exp)), u(x, y, z), (x, y, z)
            )
            .rewrite(sp.sin)
            .rewrite(sp.cos)
        )
...
def test_polynomial_forcing_uses_structured_ansatz_or_inverse():
    ...
    if result.method == "constant_coefficient_sum_split":
        ...
        assert result.details["annihilator_style"] in {
            "truncated_inverse",
            "polynomial_ansatz",
        }
    assert sp.simplify(_residual(eq, result.solution, u(x, t), (x, t))) == 0
