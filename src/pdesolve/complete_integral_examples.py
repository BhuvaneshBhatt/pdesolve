...
def run_demo():
    ...
    examples.append(
        (
            "Charpit autonomous",
            solve_charpit_complete_integral_2vars(eq1, u(x, y), (x, y)),
        )
    )
    ...
    examples.append(
        ("Charpit dp/dx", solve_charpit_complete_integral_2vars(eq2, u(x, y), (x, y)))
    )

    x, y, z = sp.symbols("x y z", real=True)
    w = sp.Function("w")
    eq3 = sp.Eq(
        sp.diff(w(x, y, z), x) ** 2, sp.diff(w(x, y, z), y) + sp.diff(w(x, y, z), z)
    )
    examples.append(
        (
            "Jacobi autonomous",
            solve_jacobi_complete_integral(eq3, w(x, y, z), (x, y, z)),
        )
    )
